...
@objc (MSHFSiriView) final public class MSHFSiriView: MSHFView {
    ...
    override internal func initializeWaveLayers() {
        ...
        rWaveLayer = MSHFJelloLayer()
        subwaveLayer = MSHFJelloLayer()
        rSubwaveLayer = MSHFJelloLayer()
        subSubwaveLayer = MSHFJelloLayer()
        rSubSubwaveLayer = MSHFJelloLayer()
    ...
    override internal func redraw() {
        ...
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + (0.25 * Double(NSEC_PER_SEC)), execute: {
